...
class LoginForm(FlaskForm):
    username = StringField('Логин или e-mail', validators=[DataRequired()],
                           render_kw={"class": "form-control", "placeholder": "Логин или e-mail"})
...
class RegistrationForm(FlaskForm):
    ...
    fio = StringField('Ваше имя', validators=[DataRequired()],
                      render_kw={"class": "form-control"})
    password = PasswordField('Пароль', validators=[DataRequired()], render_kw={"class": "form-control"})
    password2 = PasswordField('Повторите пароль', validators=[DataRequired(),
                              EqualTo('password')], render_kw={"class": "form-control"})
    company = StringField('Организация', validators=[DataRequired()],
                          render_kw={"class": "form-control"})
